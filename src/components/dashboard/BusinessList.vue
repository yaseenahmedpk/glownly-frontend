<script setup>
import Vue3EasyDataTable from 'vue3-easy-data-table';
import 'vue3-easy-data-table/dist/style.css';
import { computed } from 'vue';
import { onBeforeRouteLeave } from "vue-router";
import { useI18n } from 'vue-i18n';
import { hasPermission } from '../../helpers/authHelper';
import { updateBusinessStatus } from '../../services/businessService';
import { showErrorAlert } from '../../helpers/swal';
import { handleApiError } from '../../helpers/handleApiError';
import { useToast } from '../../helpers/useToast';
import Swal from 'sweetalert2';

const { t } = useI18n();
const { toast } = useToast();

defineProps({
    businesses: {
        type: Array,
        default: () => [],
    },
    loading: Boolean,
});
onBeforeRouteLeave(() => {
    document.querySelectorAll(".tooltip").forEach(el => el.remove());
});
const emit = defineEmits(['businessData', 'deleteBusiness', 'statusUpdated', 'viewBusiness'])

const selectBusiness = (business) => {
    emit('businessData', {
        id: business.id,
        business_name: business.business_name,
        account_type_name: business.account_type_name,
        service_type_name: business.service_type_name,
        status_name: business.status_name,
        city: business.city,
        branch: business.branch,
    });
}

const viewBusiness = (business) => {
    emit('viewBusiness', business);
}

const deleteBusiness = (business) => {
    emit('deleteBusiness', business.id);
}

const toggleStatus = async (business) => {
    const newStatus = business.status === 1 ? false : true;
    const actionText = newStatus ? 'activate' : 'deactivate';

    const result = await Swal.fire({
        title: t('are_you_sure'),
        text: newStatus ? `Are you sure you want to activate ${business.business_name}?` : `Are you sure you want to deactivate ${business.business_name}?`,
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#d33',
        cancelButtonColor: '#3085d6',
        confirmButtonText: t('yes'),
        cancelButtonText: t('cancel')
    });

    if (result.isConfirmed) {
        try {
            await updateBusinessStatus(business.id, newStatus);
            emit('statusUpdated', business.id);
            toast(t(newStatus ? 'business_activated_successfully' : 'business_deactivated_successfully'));
        } catch (error) {
            showErrorAlert(handleApiError(error, t));
        }
    }
}

const headers = computed(() => {
    return [
        { text: t('id'), value: 'id', sortable: true },
        { text: t('business_name'), value: 'business_name', sortable: true },
        { text: 'Account Type', value: 'account_type_name', sortable: true },
        { text: 'Service Type', value: 'service_type_name', sortable: true },
        { text: t('status'), value: 'status_name', sortable: false },
        { text: t('city'), value: 'city', sortable: true },
        { text: 'Branches', value: 'branch', sortable: false },
        { text: t('action'), value: 'action', sortable: false },
    ];
});
</script>

<template>
    <Vue3EasyDataTable :headers="headers" :items="businesses" :loading="loading" :search-field="['id', 'business_name', 'city']"
        :search-value="''" :sort-by="'id'" :sort-type="'asc'" :rows-per-page="10"
        :rows-per-page-message="'Rows per page:'" :rows-of-page-separator-message="'of'"
        :empty-message="'No data found'" :buttons-pagination="true" theme-color="#007bff"
        table-class-name="customize-vue-table table table-bordered">
        <template #item-branch="item">
            <span>{{ item.branch?.length || 0 }}</span>
        </template>
        <template #item-status_name="item">
            <span :class="item.status === 1 ? 'text-success' : 'text-danger'">
                {{ item.status === 1 ? t('active') : t('inactive') }}
            </span>
        </template>
        <template #item-action="item">
            <div>
                <svg v-if="hasPermission('can_edit_business')"
                    xmlns="http://www.w3.org/2000/svg" width="18" style="color:#007bff;cursor: pointer;"
                    @click="viewBusiness(item)" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                    class="size-6" v-tooltip="{ text: $t('view'), placement: 'left' }">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
                <svg v-if="hasPermission('can_edit_business')"
                    xmlns="http://www.w3.org/2000/svg" width="18" style="color:green;cursor: pointer;"
                    @click="selectBusiness(item)" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                    class="size-6 ml-2" v-tooltip="{ text: $t('edit'), placement: 'left' }">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                </svg>
                <svg v-if="hasPermission('can_edit_business')"
                    xmlns="http://www.w3.org/2000/svg" width="18" :style="{ color: item.status === 1 ? 'orange' : 'blue', cursor: 'pointer' }"
                    @click="toggleStatus(item)" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                    class="size-6 ml-2" v-tooltip="{ text: item.status === 1 ? t('deactivate') : t('activate'), placement: 'top' }">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182" />
                </svg>
                <svg v-if="hasPermission('can_delete_business')"
                    xmlns="http://www.w3.org/2000/svg" width="18" style="color:red;cursor: pointer;"
                    @click="deleteBusiness(item)" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                    class="size-6 ml-2" v-tooltip="{ text: $t('delete'), placement: 'top' }">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                </svg>
            </div>
        </template>
    </Vue3EasyDataTable>
</template>